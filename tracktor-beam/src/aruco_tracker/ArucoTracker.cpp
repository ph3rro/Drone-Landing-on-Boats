	...
	: Node("aruco_tracker_node")
{
	...
	_image_sub = create_subscription<sensor_msgs::msg::Image>(
			     "/world/waves/model/x500_gimbal_0/link/camera_link/sensor/camera/image", qos, std::bind(&ArucoTrackerNode::image_callback, this, std::placeholders::_1));

	_camera_info_sub = create_subscription<sensor_msgs::msg::CameraInfo>(
				   "/world/waves/model/x500_gimbal_0/link/camera_link/sensor/camera/camera_info", qos, std::bind(&ArucoTrackerNode::camera_info_callback, this, std::placeholders::_1));
	...
	_small_marker_detected_pub = create_publisher<std_msgs::msg::Bool>("/small_marker_detected", qos);
	
	//RCLCPP_INFO(get_logger(), "Publisher created for /small_marker_detected topic");
}
...
void ArucoTrackerNode::loadParameters()
{
	...
	// In loadParameters()
	declare_parameter<int>("small_aruco_id", 1); // Default to 2
	
	declare_parameter<double>("marker_size", 0.5);
	declare_parameter<double>("small_marker_size", 0.0564971751);	
	...
	get_parameter("small_aruco_id", _param_small_aruco_id);
	get_parameter("small_marker_size", _param_small_marker_size);
}
...
void ArucoTrackerNode::image_callback(const sensor_msgs::msg::Image::SharedPtr msg)
{
	try {
		...
		if (!_camera_matrix.empty() && !_dist_coeffs.empty()) {
			...
			int target_index = -1;
			double current_marker_size = 0.0;
			bool small_marker_detected = false;

			// --- PRIORITY LOGIC ---
			
			// 1. Search for the Small Marker (High Priority)
			for (size_t i = 0; i < ids.size(); i++) {
				//RCLCPP_INFO(get_logger(), "Checking marker ID: %d", ids[i]);
				if (ids[i] == _param_small_aruco_id) {
					target_index = i;
					current_marker_size = _param_small_marker_size;
					small_marker_detected = true;
					//RCLCPP_INFO(get_logger(), "Small marker (ID=%d) detected!", _param_small_aruco_id);
					break; // Stop looking, we found the best one!
				}
			}

			// 2. If Small Marker not found, search for Big Marker (Fallback)
			if (target_index == -1 && small_marker_detected == false) {
				for (size_t i = 0; i < ids.size(); i++) {
					if (ids[i] == _param_aruco_id) {
						target_index = i;
						current_marker_size = _param_marker_size;
						break; // Stop looking, we found the backup.
					}
				}
			}
			...
			if (target_index != -1) {
				...
				cv::Vec3d rvec, tvec;
				cv::solvePnP(objectPoints, undistortedCorners[target_index], _camera_matrix, cv::noArray(), rvec, tvec);
				// Annotate the image
				cv::drawFrameAxes(cv_ptr->image, _camera_matrix, cv::noArray(), rvec, tvec, current_marker_size);
				...
				// Publish small marker detection status
				std_msgs::msg::Bool small_marker_msg;
				small_marker_msg.data = small_marker_detected;
				_small_marker_detected_pub->publish(small_marker_msg);
				if (small_marker_detected) {
					RCLCPP_INFO(get_logger(), "Published small_marker_detected: true");
				}
				...
				// No marker detected - publish false
				std_msgs::msg::Bool small_marker_msg;
				small_marker_msg.data = false;
				_small_marker_detected_pub->publish(small_marker_msg);
			}
